...
def distances_to_all(query, X_train):
    ...
    row of X_train, computed in one broadcasted call 
    """
...
def knn_predict(query, X_train, y_train, k):
    ...
    # Indices of the k smallest distances. using argpartition insdtead of argsort
    k = min(k, len(dists))
    nearest_idx = np.argpartition(dists, k - 1)[:k]    #argpartition is O(n) 
