...
# Sanity check: distances_to_all is euclidean_distance's formula reused
# (broadcast across all rows), not a separately-written one — confirm
# the vectorized batch result matches the scalar function row by row.
...
# centroids (not the query, which is the zero vector and would trivially
# return 0.0 regardless of direction — a degenerate, uninformative case).
